...
$MainFiles = @(Get-ChildItem -LiteralPath $LatexDir -Filter "main-*.tex" -File | Sort-Object Name)
...
try {
    foreach ($MainFile in $MainFiles) {
        & xelatex -interaction=nonstopmode -halt-on-error $MainFile.Name | Out-Null
