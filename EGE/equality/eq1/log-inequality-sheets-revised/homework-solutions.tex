...
\begin{criterionbox}
  Проверяется не воспроизведение шаблона, а перенос: сначала условие на
  простое выражение, затем то же рассуждение для \(x^2-4\).
\end{criterionbox}
...
\begin{teacheranswer}[Задание 6]
  ...
  \[
    16(t^2-t).
  \]
  Вариант Б использует основание \(2\) вместо десятичного логарифма.
  В варианте В имеем
  \[
    t=\lg(|x^2-1|^2)=2\lg|x^2-1|,
  \]
  поэтому исходное выражение принимает вид \(t^4-4t^2\) и остаётся
  многочленом четвёртой степени относительно \(t\).
\end{teacheranswer}
...
\begin{criterionbox}[Итоговая диагностика]
  ...
  алгебраическое ядро; задание 4 --- отдельный перенос способа решения
  двойного неравенства с модулем; задание 5 --- сборку метода с опорой;
  задание 6 --- выбор замены; задание 7 --- перенос с минимальной опорой;
  задание 8 --- полностью самостоятельное решение; задание 9 --- обобщение.
\end{criterionbox}
