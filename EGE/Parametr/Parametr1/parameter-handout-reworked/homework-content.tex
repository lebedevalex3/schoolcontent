...
\begin{routebox}
  ...
  \begin{enumerate}[label=\arabic*.,leftmargin=1.8em,itemsep=0.2em,topsep=0.25em]
    ...
    \item определите условие нужного количества нулей;
    \item проверьте каждый кандидат в знаменателе;
...
\begin{homeworktask}[Ноль дроби]{1}
  ...
  \[
    \frac{(x-2)(x+3)}{x-2}=0.
  \]
  Объясните, почему один из нулей числителя не является корнем дроби.
\end{homeworktask}
\answerspace{16mm}
...
\begin{homeworktask}[Подробная опора]{3}
...
\begin{homeworktask}[Опора сокращается]{4}
  Найдите все значения \(a\), при которых уравнение
  \[
...
\begin{homeworktask}[Найдите точный пробел в решении]{5}
  Ученик написал:
  \begin{quote}
    «Числитель уравнения
    \(
...
\begin{homeworktask}[Полная классификация]{6}
  ...
  \]
  определите количество корней при всех значениях параметра \(a\).
