...
\begin{checkpointbox}[Входная диагностика: 3--4 минуты]
  ...
  \begin{enumerate}[label=\textbf{\arabic*.},itemsep=0.45em]
    \item Решите уравнение
      \[
        \frac{x-3}{x+1}=0.
      \]
      Ответ: \(x=\answerblank[18mm]\).

    \item Раскройте модуль:
      \[
        |x|=
        \begin{cases}
          \answerblank[18mm],&x\ge0,\\[1mm]
          \answerblank[18mm],&x<0.
        \end{cases}
      \]

    \item Подставьте \(x=a+1\) в выражение \(x^2-x-a\) и упростите:
      \[
        (a+1)^2-(a+1)-a=\answerblank[25mm].
      \]
  \end{enumerate}
\end{checkpointbox}

\begin{reflectionbox}[Самопроверка готовности]
  Если все три задания выполнены уверенно, переходите к основной задаче.
  Если возникло затруднение, отметьте его:
  \choicebox\ ноль дроби\qquad
  \choicebox\ модуль\qquad
  \choicebox\ подстановка.
\end{reflectionbox}
...
Получите:
\[
  D(x_2)=\frac{\answerfraction[38mm]}{25}.
\]
...
\begin{checkpointbox}[Обязательная часть завершена]
  На основном уроке достаточно выполнить разделы до этого блока: исследовать
  числитель, проверить оба найденных значения в знаменателе и записать
  окончательный ответ.

  Следующие разделы относятся к расширению. Их можно выполнить на втором
  уроке, дома или как дополнительные задания.
\end{checkpointbox}

\newpage
\section*{Расширение и углубление}

В этой части вы проверите ответ на отдельных значениях параметра, объясните
результат графически, познакомитесь с более коротким способом и попробуете
восстановить решение без подробных подсказок.
...
Тогда нули числителя определяются уравнением \(3|x|-2x=b\).
...
Этот раздел показывает, как сразу найти значения параметра, при которых один
из нулей числителя обращает знаменатель в ноль и поэтому не является корнем
исходного уравнения.
...
\begin{questionsbox}[Перед завершением работы]
  \begin{itemize}[label=\(\square\)]
    ...
    \item Я различаю нули числителя и корни исходного уравнения.
    \item Я подставил каждый найденный ноль числителя в знаменатель.
