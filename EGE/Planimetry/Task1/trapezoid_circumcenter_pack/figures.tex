...
\begin{tikzpicture}[scale=0.55,every node/.style={geo label}]
  ...
  % Продолжения NA и ND — тонкие, вспомогательные
  \draw[geo main] (A)--(N) (D)--(N);

  % Стороны трапеции ABCD — жирные
...
\begin{tikzpicture}[scale=0.60,every node/.style={geo label}]
  ...
  \pic[geo angle,"$64^\circ$"{geo angle label},angle radius=7mm,angle eccentricity=0.7]
    {angle=N--C--B};
  % 128° при M — двумя дужками (ответ на вопрос 3)
  \pic[geo angle,"$128^\circ$"{geo angle label},angle radius=9mm,angle eccentricity=0.7]
    {angle=N--M--B};
  \pic[geo angle,angle radius=10mm]
    {angle=N--M--B};
\end{tikzpicture}
\end{center}%
}
...
\begin{tikzpicture}[scale=0.68,every node/.style={geo label}]
  ...
  \pic[geo angle,"$128^\circ$"{geo angle label},angle radius=7.5mm,angle eccentricity=0.7]
    {angle=N--M--B};
