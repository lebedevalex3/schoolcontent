...
% Ученик сам продлевает AB и DC и ставит точку N.
% -------------------------------------------------
...
\begin{tikzpicture}[scale=0.55,every node/.style={geo label}]
  ...
  % Только стороны трапеции — жирные
  \draw[geo strong] (A)--(B)--(C)--(D)--cycle;
  ...
  % Невидимая точка выше трапеции — расширяет bounding box,
  % чтобы ученик мог дорисовать точку N и продолжения AB, DC.
  \path (3.31, 7);
\end{tikzpicture}
\end{center}%
}
...
\begin{center}
\begin{tikzpicture}[scale=0.48,every node/.style={geo label}]
  \coordinate (B) at (0,0);
  \coordinate (C) at (8,0);
  \coordinate (N) at (3.31,9.615);
  \coordinate (A) at ($(B)!0.5!(N)$);
...
\begin{center}
\begin{tikzpicture}[scale=0.42,every node/.style={geo label}]
  \coordinate (B) at (0,0);
  \coordinate (C) at (8,0);
  \coordinate (N) at (3.31,9.615);
  \coordinate (M) at (4,4);
  ...
  % 64° при C — одной дужкой (данное из условия)
  \pic[geo angle,"$64^\circ$"{geo angle label},angle radius=5mm,angle eccentricity=1.22]
    {angle=N--C--B};
  % 128° при M — двумя дужками (ответ на вопрос 3)
  \pic[geo angle,"$128^\circ$"{geo angle label},angle radius=5.5mm,angle eccentricity=1.42]
    {angle=N--M--B};
  \pic[geo angle,angle radius=6.5mm]
    {angle=N--M--B};
\end{tikzpicture}
\end{center}%
}
...
\begin{center}
\begin{tikzpicture}[scale=0.45,every node/.style={geo label}]
  \coordinate (B) at (0,0);
  \coordinate (M) at (4,4);
  \coordinate (N) at (3.31,9.615);

  \draw[geo main] (N)--(M)--(B)--cycle;
  \node[geo point,label={[geo label]below left:$B$}] at (B) {};
  \node[geo center point,label={[geo label]right:$M$}] at (M) {};
  \node[geo point,label={[geo label]above:$N$}] at (N) {};

  \pic[geo angle,"$128^\circ$"{geo angle label},angle radius=5mm,angle eccentricity=1.42]
    {angle=N--M--B};
...
\begin{tikzpicture}[scale=0.70,every node/.style={geo label}]
  ...
  \pic[geo angle,"$26^\circ$"{geo angle label},angle radius=9mm,angle eccentricity=1.28]
    {angle=M--B--N};
  % 45° (CBM) — двумя дужками
  \pic[geo angle,"$45^\circ$"{geo angle label},angle radius=15mm,angle eccentricity=1.18]
    {angle=C--B--M};
  \pic[geo angle,angle radius=14mm]
    {angle=C--B--M};
  % 71° (CBN) убрано — это то, что ученик должен сложить
\end{tikzpicture}
\end{center}%
}
